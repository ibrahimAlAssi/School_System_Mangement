...
class AddParent extends Component
{
    ...
    public $catchError,$show_table = true, $updateMode = false,$Parent_id;
    ...
    public function submitForm_edit()
    {
        ...
        if ($this->Parent_id) {
            ...
            $parent->update([
                ...
                'Job_Mother'=>['en'=>$this->Job_Mother_en,$this->Job_Mother],
    ...
    {
        $myParent=My_Parent::find($id);//to konw National_ID_father
        $parent_Attachments=ParentAttachment::where('parent_id',$id)->first();//Know If I have Attachments
        if($parent_Attachments)
        {
            Storage::disk('parent_attachments')->deleteDirectory($myParent->National_ID_Father);//deleteFolder
            ParentAttachment::where('parent_id',$id)->delete();
        }
        My_Parent::findOrFail($id)->delete();//delete my_Parent
        return redirect()->to('/add_parent');
    }
    //clearForm
